...
public class ClientDateFrame extends JFrame {
	...
	private JLabel lblCharacterValue;
	...
	public void updateTotalCount (int val) {	
		
		this.lblCharacterValue.setText(String.valueOf(val));			
	}
	...
	private JPanel getCharacterPanel(Font font) {
		
		// Create component to display date retrieve from the server
		JPanel panel = new JPanel();
		JLabel lblCharCount = new JLabel ("Total Character (from Server): ");		
		
		// Style the component
		lblCharCount.setFont(font);
		lblCharacterValue.setFont(font);
		lblCharCount.setBackground(Color.WHITE);
		lblCharCount.setOpaque(true);
		lblCharacterValue.setBackground(Color.WHITE);
		lblCharacterValue.setOpaque(true);

		// Organize components into panel		
		panel.add(lblCharCount);
		panel.add(lblCharacterValue);
	...
	private void loadComponent() {
		...
		// Get server total character's panel and add to frame
		JPanel center = getCharacterPanel(font);
